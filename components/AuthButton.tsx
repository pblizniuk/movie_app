...
export default async function AuthButton() {
  ...
  return !user ? (
    <Link
      href="/login"
      className="shadow-light-3 hover:shadow-light-2 focus:shadow-light-2 active:shadow-light-2 inline-block rounded bg-lime-500 px-2 py-2 font-semibold leading-normal text-white transition duration-150 ease-in-out hover:bg-lime-600 focus:bg-lime-600 focus:outline-none focus:ring-0 active:bg-lime-600 motion-reduce:transition-none md:px-6"
    >
      Log In
    </Link>
  ) : (
    <span className="group relative">
      ...
      <div className="absolute right-0 top-3 hidden min-w-[200px] rounded-md bg-stone-600/90 px-3 py-2 text-foreground/80 group-hover:block">
